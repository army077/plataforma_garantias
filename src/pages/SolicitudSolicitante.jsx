...
export default function SolicitudShow() {
  ...
  const mutCambiar = useMutation({
    mutationFn: ({ a, nota }) => cambiarEstadoSolicitud(id, a, nota, 1),
  ...
  const totalPreview = selected ? (qtyValid ? (qtyNum * (selected.precio_venta ?? 0)) : 0) : 0;
        ...
        <div className="grid md:grid-cols-2 gap-2 mt-3">
          {productos.map((p)=>(
            ...
            >
              ...
              <div className="text-sm text-neutral-400">
                {p.uni_med} • Precio: ${p.precio_venta ?? 0}
                {selected?.id === p.id && qtyValid ? (
                  <span className="ml-2 text-neutral-300">
                    • Cant: {qtyNum} • Total: ${totalPreview.toFixed(2)}
                  </span>
                ) : null}
              </div>
            </div>
