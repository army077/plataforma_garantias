...
export default function SolicitudShow() {
  ...
  const mutDeleteItem = useMutation({
    mutationFn: (itemId) => deleteItem(id, itemId, usuarioId),
    onSuccess: () => queryClient.invalidateQueries(["solicitud", id]),
    onError: (e) => alert("Error al eliminar item: " + (e?.response?.data?.error || e.message)),
  });
            ...
            {s.items.map((it) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Footer */}
                  <div className="mt-4 flex justify-between items-center text-xs text-slate-500 border-t border-slate-200 pt-2">
                    {(role === "admin" || role === "garantias") && it.estado_pieza_code === "SOLICITADA" && (
                      <button
                        className="flex items-center gap-1 text-red-500 hover:text-red-700 hover:bg-red-50 px-2 py-1 rounded transition-colors"
                        disabled={mutDeleteItem.isLoading && mutDeleteItem.variables === it.id}
                        onClick={() => {
                          if (window.confirm(`¿Eliminar la pieza ${it.numero_parte} - ${it.descripcion}?`))
                            mutDeleteItem.mutate(it.id);
                        }}
                      >
